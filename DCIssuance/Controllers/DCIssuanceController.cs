...
namespace DNPIssuance.Controllers
{
    [Route("api/[controller]/[action]")]
    [ApiController]
    [Authorize(AuthenticationSchemes = AuthenticationSchemes.JWT_BEARER_TOKEN_STATELESS)]
    public class DCIssuanceController : ControllerBase
    {
        private readonly IDCIssuanceService _service;
        public VwUser User
        {
            get
            {
        ...
        public DCIssuanceController(IDCIssuanceService service)
        {
        ...
        public async Task<ApiResponse> GetApplicationsAgainstRegNo(string registrationNo)
        {
            ...
            {
                owner.Persons = SharedLib.Common.Extentions.ToList<ImPerson>(ds.Tables[1]).FirstOrDefault();
                owner.Business = SharedLib.Common.Extentions.ToList<ImBusiness>(ds.Tables[2]).FirstOrDefault();
                if (owner.Persons is not null)
                { 
                    owner.Persons.PhoneNumbers = SharedLib.Common.Extentions.ToList<ImPhoneNumber>(ds.Tables[3]).ToList();
                }
        ...
        [HttpGet(Name = "GetAssesmentDCIssuance")]
        public async Task<ApiResponse> GetAssesmentDCIssuance(long applicationId, long assesmentBaseId)
        {
            this._service.VwUser = this.User;

            var ds = await this._service.GetAssesmentDCIssuance(applicationId, assesmentBaseId, this.User.UserId);
        ...
        [HttpGet(Name = "GetDCApplicationDetail")]
        public async Task<ApiResponse> GetDCApplicationDetail(long applicationId)
        {
            this._service.VwUser = this.User;
            ...
            if (Owner is not null)
            {
                ...
                //var assesment = SharedLib.Common.Extentions.ToList<ImAssessment>(ds.Tables[8]).ToList(); //ds.Tables[8].ToList<ImAssessment>(); 
        ...
        [HttpPost(Name = "AddDCIssuance")]
        public async Task<ApiResponse> AddDCIssuance(ImDuplicateCard imDuplicateCard)
        {
            this._service.VwUser = this.User;

            var ds = await this._service.SaveDCIssuance(imDuplicateCard, this.User.UserId);
